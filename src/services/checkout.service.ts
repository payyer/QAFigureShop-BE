...
export class CheckoutService {
    static async validateCheckout(items: { productId: string, quantity: number }[], voucherCode?: string) {
        let subtotal = 0;
        ...
        for(const item of items) {
            ...
            if(!product) throw new  AppError(`Sản phẩm ${item.productId} không tồn tại`, 404);
            if(product.stock < item.quantity) throw new  AppError(`Sản phẩm ${item.productId} trong kho không đủ đáp `, 400);
        ...
        if (voucherCode) {
            ...
            if (!voucher || !voucher.isAvailable) throw new AppError("Mã giảm giá không tồn tại hoặc đã hết lượt sử dụng", 400)
            voucherInfo = voucher
